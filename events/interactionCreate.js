      ...
      interaction.message.edit({
        components: [
          new MessageActionRow().addComponents(
            new MessageButton().setStyle("SUCCESS").setLabel("Şipariş Tamamlandı").setEmoji("😀").setCustomId(`tamamlandı`),
            new MessageButton().setLabel("Şiparişi İptal Et").setEmoji("😀").setCustomId(`iptal`).setStyle("DANGER"),
            new MessageButton().setLabel("Promosyon Kodu Kullanıldı").setEmoji("😀").setCustomId(`promosyon`).setStyle("SECONDARY").setDisabled(true),
          )
        ],
        embeds: [
          {
            ...
            fields: [
              ...
              { name: "Ödeme Yöntemi", value: `${urun.yontem == "papara" ? "😀 Papara \n↬**" : "😀 İtemsatış"}`, inline: false },
  ...
  if (interaction.isSelectMenu()) {
        ...
        knl.send(
          {
            ...
            content: `${interaction.member} Siparişin şuanda onay bekliyor, bu süreçte sormak istediğin merak ettiğin bir şey varsa bize söylemekten çekinme!`,
            embeds: [
              {
                title: "Şiparişiniz hazırlanıyor -" + iskod,
                description: `**${urun.name}** adlı ürün siparişiniz ödeme tamamlandıktan sonra yetkili üzerinden verilecektir`,
                fields: [
                  ...
                  { name: "Ödeme Yöntemi", value: `${tur == "papara" ? "😀 Papara \n↬**" : "😀 İtemsatış"}`, inline: false }
                ],
    ...
    if (interaction.customId == `ürün-liste`) {
      ...
      let embed = new MessageEmbed()
        ...
        .setFooter({ text: `Ürün Kodu: ${_data.urunKodu} | Developed by VipAhmetAgaKeser#9982` })
    ...
    if (interaction.customId == `ayarlar-${interaction.member.id}`) {
      ...
      if (interaction.values[0].toString() == "düzenle") {
        ...
        interaction.reply({
          ephemeral: true, components: [new MessageActionRow().addComponents(
  ...
  if (interaction.isButton()) {
      ...
      interaction.reply({
        ephemeral: true, content: "Siparişi iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz", components: [
          new MessageActionRow().addComponents(new MessageButton()
            .setCustomId("iptaledildi").setLabel("Evet! İptal Et").setStyle("DANGER").setEmoji("😀"))
        ]
      })
      return;
    }
      ...
      interaction.message.edit({
        components: [
          new MessageActionRow().addComponents(
            new MessageButton().setStyle("SUCCESS").setLabel("Şipariş Tamamlandı").setEmoji("😀").setCustomId(`tamamlandı`),
            new MessageButton().setLabel("Şiparişi İptal Et").setEmoji("😀").setCustomId(`iptal`).setStyle("DANGER"),
            new MessageButton().setLabel("Promosyon Kodu Kullan").setEmoji("😀").setCustomId(`promosyon`).setStyle("SECONDARY"),
          )
        ]
      })
    ...
    if (interaction.customId == "sepetigör") {
      ...
      **İndirim Bilgisi:** ${indMsg}
      --------------------------------------------------------------------------------
      **😀 Papara ile öderseniz: ${(total - (total * 7 / 100)).toString().slice(0, 4)}TL**
      **😀 İtemsatış ile öderseniz: ${(total + (total * 7 / 100) + (total * 5 / 100)).toString().slice(0, 4)}TL**`)

      const row = new MessageActionRow().addComponents(
        new MessageSelectMenu().setCustomId(`ayarlar-${interaction.member.id}`).setPlaceholder("Sepet Ayarları")
          .setMaxValues(1).setMinValues(1).setOptions([
            { label: "Papara ile ödemeyi tamamla", description: "Papara ile ödemek istiyorum", value: "papara", emoji: "😀" },
            { label: "İtemsatış ile ödemeyi tamamla", description: "İtemsatış ile ödemek istiyorum", value: "itemsatis", emoji: "😀" },
            { label: "Sepeti Düzenle", description: "sepetimi düzenlemek istiyorum", value: "düzenle", emoji: "😀" },
            { label: "Sepeti Boşalt", description: "Sepetimdeki tüm ürünleri silmek istiyorum", value: "hepsinisil", emoji: "😀" },
          ])
      )
        ...
        if (tur == "al") {
          const row = new MessageActionRow().addComponents(
            new MessageSelectMenu().setCustomId(`satinal-${_data.urunKodu}`).setPlaceholder("Ödeme Yönetmini Seçiniz")
              .setMaxValues(1).setMinValues(1).setOptions([
                { label: "Papara ile öde (%7 indirim!)", description: "Papara ile ödemek istiyorum", value: "papara", emoji: "😀" },
                { label: "İtemsatış ile öde (ek ücret tahsil edilir)", description: "İtemsatış ile ödemek istiyorum", value: "itemsatis", emoji: "😀" },
              ])
          )
          interaction.reply({
